...
def firebase_url(path: str) -> str:
    """Constructs the full Firebase REST URL."""
    path = path.strip('/')
    return f'{FIREBASE_URL}/{path}.json'

def firebase_get(path: str):
    """Performs a GET request to Firebase."""
...
def firebase_post(path: str, value):
    """Performs a POST request to Firebase."""
    url = firebase_url(path)
    try:
...
def firebase_patch(path: str, value_dict: dict):
    """Performs a PATCH request to Firebase."""
    url = firebase_url(path)
    try:
...
# Graphing helpers
# -----------------------
def get_last_n_days_data(user_data, n):
    """Retrieves intake data for the last n days."""
    today = datetime.date.today()
    days = []
    values = []
    for i in range(n):
        day = today - datetime.timedelta(days=i)
        # key = day.strftime('%Y-%m-%d')  # not strictly needed
        key_str = day.isoformat()
        days.append(day.strftime('Day %d' % (n - i)))  # Correctly label as "Day 1" is the oldest
        values.append(user_data.get(key_str, 0))

    # The original logic intended to show oldest → newest, so we keep the reversal:
    return days[::-1], values[::-1]

def show_week_graph(user_data):
    """Displays a bar graph of the last 7 days' intake."""
    days, values = get_last_n_days_data(user_data, 7)
    fig, ax = plt.subplots()
    ax.bar(days, values, color='#67b3df')
    ax.set_title("This Week's Hydration")
    ax.set_xlabel('Day')
    ax.set_ylabel('Water (ml)')
    st.pyplot(fig)

def show_month_graph(user_data):
    """Displays a line graph of the last 30 days' intake."""
    days, values = get_last_n_days_data(user_data, 30)
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(days, values, marker='o', color='#3498db')
    ax.set_title("This Month's Hydration (30 days)")
    ax.set_xlabel('Day')
    ax.set_ylabel('Water (ml)')
...
def create_user(username: str, password: str):
    ...
    payload = {
        ...
        'days': {} # Initialize days node for hydration data
    }
...
def get_today_intake(uid: str):
    """Gets the user's total water intake for the current day."""
    if not uid:
        return 0

    # Primary check in the structured 'days' node
    ...
    # Fallback/Legacy check for older root field (kept for robustness)
...
def set_today_intake(uid: str, ml_value: int):
    """Sets the user's water intake for the current day."""
    if not uid:
        return False

    ml = int(max(0, ml_value))
    path = f'{USERS_NODE}/{uid}/days/{DATE_STR}'
    # The patch includes the intake for the specific date
    return firebase_patch(path, {'intake': ml})

def reset_today_intake(uid: str):
    """Resets the user's intake to 0 for the current day."""
    return set_today_intake(uid, 0)

def get_user_profile(uid: str):
    """Retrieves the user's profile and goal settings."""
...
def get_username_by_uid(uid: str):
    """Retrieves the username for a given UID."""
...
def apply_theme(theme_name: str):
    """Applies custom CSS for different themes."""
    # ---------------------- LIGHT MODE ----------------------
    if theme_name == 'Light':
        st.markdown("""
        <style>
        ...
        div[data-testid="metric-container"] div[data-testid="stMetricValue"] > span, div[data-testid="metric-container"] div[data-testid="stMetricValue"] span { color: inherit !important; /* makes it use theme color */ }
        </style>
...
def dashboard_ui():
    ...
    user_data_all = firebase_get(f'{USERS_NODE}/{uid}/days')
    if not isinstance(user_data_all, dict):
        user_data_all = {} # Ensure it's a dict for graphing functions
    ...
    with left_col:
        ...
        try:
            ...
            idx = 0  # default to 'Light'
            st.session_state.theme = theme_options[0] # ensure state is set

        theme_choice = st.selectbox('Theme', theme_options, index=idx)
        if theme_choice != st.session_state.theme:
            st.session_state.theme = theme_choice
            apply_theme(theme_choice) # update immediately visually (no rerun required)
    ...
    with right_col:
        ...
        if nav == 'Home':
            ...
            st.metric(
                'Total intake (ml)',
                f'{intake} ml',
                delta=f'{remaining} ml to goal' if remaining > 0 else 'Goal reached!'
            )
            ...
            st.info("Note: Data is fetched from your Firebase 'days' node.")
            
            st.subheader('Last 7 Days')
            show_week_graph(user_data_all)
            
            st.subheader('Last 30 Days')
            show_month_graph(user_data_all)
            ...
            user_goal_val = st.number_input(
                'Daily goal (ml)',
                min_value=500,
                max_value=10000,
                value=int(profile.get('user_goal_ml', suggested)),
                step=50
            )

            if st.button('Save profile', key='save_profile'):
                ok = update_user_profile(uid, {
                    'age_group': age_choice,
                    'user_goal_ml': int(user_goal_val)
                })
                if ok:
                    st.success('Profile saved.')
                    # Rerun to update the profile displayed on the Home page immediately
                    st.rerun()
                else:
